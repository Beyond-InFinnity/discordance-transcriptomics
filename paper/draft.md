...
annotation table are available at
**https://github.com/Beyond-InFinnity/discordance-transcriptomics**. The
annotation table is `data/derived/annotation/`: per-parcel coupling angle,
baseline OEF, discordance frequency by mode, dropout coverage and venous
partial-volume, with a JSON schema and per-column reliability labels, plus the
gene-set profiles behind §3.4. Derived `results/` are not tracked — they rebuild
from `scripts/regenerate_all.sh` in about four hours, and each carries a manifest
so a rebuild is checkable rather than merely repeatable. Every result artifact
